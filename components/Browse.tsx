...
import { useSubLoad } from 'hooks/useSubLoad'
...
export const Browse = () => {
  ...
  const NFTElement = ({ tokenData }: { tokenData: TokenData }) => {
    const [loaded, elRef] = useSubLoad('.list', true)

    return (
      <div key={tokenData.tokenManager?.pubkey.toString()} ref={elRef}>
        {!loaded ? (
          'Loading...'
        ) : (
          <>
            <NFT
              ...
              tokenData={tokenData}
            />
            <p className="text-white">{tokenData.metadata.name}</p>
            {
              {
                ...
                [TokenManagerState.Invalidated]: (
                  <Tag state={TokenManagerState.Invalidated}>Invalidated</Tag>
                ),
